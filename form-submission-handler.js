...
function handleFormSubmit(event) {  // handles form submit withtout any jquery
  ...
  if (!data.Contact) {
    document.getElementById('contactAlert').style.display = 'inline';
    return false;
  }
  else {
  ...
  if( !validEmail(data.Email) ) {   // if email is not valid show error
    document.getElementById('emailAlert2').style.display = 'inline';
    return false;
  }
  else {
  ...
  if(!grecaptcha.getResponse()){
    document.getElementById('captchaMessage').style.display = 'inline';
    return false;
  }
  else {
